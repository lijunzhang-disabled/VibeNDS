...
//!
//! Single SDL2 window stacking the two 256x192 screens vertically with a
//! 90 px gap (matching the real DS bezel proportion). Phase 1 only clears
//! the window — actual framebuffer rendering arrives in Phase 3.
...
/// Vertical gap between the two screens, measured in NDS pixels.
pub const SCREEN_GAP: u32 = 90;
...
impl DualScreen {
    pub fn new(sdl: &sdl2::Sdl, scale: u32) -> Self {
        ...
        let mut canvas = window.into_canvas().software().build().expect("create canvas");
        ...
        DualScreen { canvas, texture_creator, pixel_buffer, scale }
    }
    ...
    fn blit_one(&mut self, src: &[u16], dst: Rect) {
        self.convert(src);
        let mut tex = self.texture_creator
            .create_texture_streaming(PixelFormatEnum::ARGB8888, SCREEN_WIDTH as u32, SCREEN_HEIGHT as u32)
            .expect("create texture");
        tex.update(None, &self.pixel_buffer, SCREEN_WIDTH * 4).expect("update");
    ...
    pub fn present(&mut self, top: &[u16], bot: &[u16]) {
        ...
        let dst_bot = Rect::new(
            0,
            ((SCREEN_HEIGHT as u32 + SCREEN_GAP) * self.scale) as i32,
            SCREEN_WIDTH as u32 * self.scale,
            SCREEN_HEIGHT as u32 * self.scale,
        );
