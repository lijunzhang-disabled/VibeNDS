...
const PLANES: [Plane; 6] = [
    Plane::NegX, Plane::PosX,
    Plane::NegY, Plane::PosY,
    Plane::NegZ, Plane::PosZ,
];
...
fn interpolate(a: &Vertex, b: &Vertex, t_num: i64, t_den: i64) -> Vertex {
    ...
    let lerp_color = |a: u16, b: u16| -> u16 {
        ...
        let ar = chan(a, 0); let br = chan(b, 0);
        let ag = chan(a, 5); let bg = chan(b, 5);
        let ab = chan(a, 10); let bb = chan(b, 10);
    ...
    Vertex {
        ...
        tex: [
            lerp_i16(a.tex[0], b.tex[0]),
            lerp_i16(a.tex[1], b.tex[1]),
        ],
    }
}
...
pub fn clip_polygon(input: &[Vertex]) -> Option<Vec<Vertex>> {
    ...
    if current.len() < 3 { None } else { Some(current) }
}
...
mod tests {
    ...
    fn test_triangle_straddling_near_plane_clips_to_polygon() {
        // Two verts inside (z > 0), one behind (z < 0).
        let tri = vec![
            vtx(0, 0, ONE / 2, ONE, 0x001F),   // inside
            vtx(0, ONE, ONE / 2, ONE, 0x001F), // inside
            vtx(0, 0, -ONE, ONE, 0x001F),      // outside (z < 0)
        ];
        let out = clip_polygon(&tri).expect("clipped");
        // Clipping a triangle that straddles one plane produces 4 vertices.
        assert_eq!(out.len(), 4);
        // None of the output vertices should have z < 0.
        for v in &out {
            assert!(v.clip[2] >= 0, "z should be non-negative after near-plane clip, got {}", v.clip[2]);
        }
    }
    ...
    fn test_color_interpolation_along_clipped_edge() {
        ...
        assert!(has_mix, "expected at least one interpolated red+blue vertex");
    }
    ...
    fn test_triangle_crossing_two_planes() {
        ...
        let out = clip_polygon(&tri).expect("clipped");
        assert!(out.len() >= 4, "expected ≥ 4 verts after 2-plane clip, got {}", out.len());
