...
pub struct SharedState {
    ...
    /// `POWCNT1` (0x04000304, ARM9). Bit 15 controls Display A routing:
    /// 0 = Display A/Engine A on lower screen, Display B/Engine B on upper;
    /// 1 = Display A/Engine A on upper screen, Display B/Engine B on lower.
    /// Other bits gate engine power; we mostly ignore them in Phase 3.
    pub powcnt1: u16,
    ...
    /// Words generated by an in-flight card transfer that are not yet
    /// visible to the bus — they move into `slot1_data` when the
    /// `Slot1Done` event fires after the hardware transfer time elapses.
    pub slot1_pending: VecDeque<u32>,
    /// One-shot request (ARM7 cycles) for the frame loop to schedule a
    /// `Slot1Done` event. Set by `start_slot1_transfer`, which runs deep
    /// inside a bus write and has no scheduler access.
    pub slot1_delay_request: Option<u64>,
    /// Raise the Slot-1 transfer-complete IRQ when the transfer finishes,
    /// and on which CPU.
    pub slot1_complete_irq: bool,
    pub slot1_complete_irq_to_arm7: bool,
...
impl SharedState {
    pub fn new() -> Self {
        SharedState {
            ...
            slot1_busy_polls: 0,
            slot1_pending: VecDeque::new(),
            slot1_delay_request: None,
            slot1_complete_irq: false,
            slot1_complete_irq_to_arm7: false,
